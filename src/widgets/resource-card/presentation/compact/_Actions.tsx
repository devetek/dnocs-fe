...
const ActionButton = (props: ActionButtonProps) => {
  const { ref, action, isPrimary, isActive, visibleActionOnlyIcon } = props;
  ...
  const cnActionWrapper = cn(
    'border-t text-sm px-3 py-2 w-full transition-all flex items-center justify-center gap-2 group',
    !isPrimary && 'border-l',
    disabled && 'opacity-50 cursor-no-drop',
    !disabled && {
      'hover:bg-accent/20 hover:text-accent active:bg-accent/20 active:text-accent data-[active=true]:bg-accent/20 data-[active=true]:text-accent':
        variant === 'normal',
  ...
  if (visibleActionOnlyIcon && elIcon) {
    return (
      <Tooltip className="w-full" as="div" message={label} passthrough>
        <button
          ref={ref}
          disabled={disabled}
...
export default function CompactActions(props: ActionProps) {
