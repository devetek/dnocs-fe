...
import type { ResponseErrors } from '@/shared/libs/api-client/rules/types';
...
function Error(props: ResponseErrors) {
  const errorPayload = useMemo(() => {
    if (props.kind === 'aggregate') {
      return props.errors
        .map((it) => it.error.stack || it.error.message)
        .join('\n');
    }
  ...
  return (
    <div className="relative">
      ...
      <div className="absolute left-1/2 top-1/2 -translate-1/2 border p-4 pb-2 rounded-xl shadow-2xl bg-card">
        <FailedState.WallCentered errorPayload={errorPayload} />
      </div>
    </div>
  );
}

export const DomainListState = {
  Loading,
  Error,
};
