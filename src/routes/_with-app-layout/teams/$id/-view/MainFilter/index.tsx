  ...
  return (
    <ViewModeSO
      ...
      onClickOption={(newViewMode) =>
        emit('@team-members/filters/view-mode--change', newViewMode)
      }
    />
  );
};
...
const SlotRefresh = () => {
  ...
  return (
    <Button
      ...
      buttonStyle="ghost"
      onClick={() => emit('@team-members/data--refresh', null)}
    >
      <RefreshCwIcon />
    </Button>
  );
...
export default function MainFilter() {
  return (
    <Card className="bg-card/30 rounded-xl p-1.5 flex items-center justify-end gap-x-1">
      <SlotRefresh />
      <SlotViewMode />
    </Card>
  );
