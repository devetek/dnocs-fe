...
export const [FilterModelProvider, useFilterModel] = buildSelector(
  ...
  useSubscribe('@teams/filters/search--input', (input) => {
    updateQs('q', input || undefined);
  });

  useSubscribe('@teams/filters/view-mode--change', (newViewMode) => {
    updateQs('viewMode', newViewMode !== 'auto' ? newViewMode : undefined);
  });

  useSubscribe('@teams/filters/pagination--set', (newPage) => {
